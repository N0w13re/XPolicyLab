...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **1/42**. First cell `play_stacking_toy` 0/50 (150 three-camera videos). In-progress 12/240 (5.00%). GPU0 `build_tower` stepping; GPU5 next `fill_pen_holder` (08:36). Wrist YMAX on finished cells ≫30 (`results/g05-wrist-ymax.json`). Native `num_envs=5`. | not started |
