...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **36/42** (1800 ep). Leftover in-flight: `fold_clothes`, `insert_key`, `deposit_coin`, `push_T`, `stack_blocks`, `general_pickup`. Partial Average **13.46%** (**not vs 14.88** until 42/42). Elastic stopped so idle GPUs will not pick Xiaomi. Native `--num_envs 5`. | not started |
