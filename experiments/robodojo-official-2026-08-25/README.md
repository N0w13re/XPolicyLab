...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **14/42**. New: `pick_from_conveyor_by_image` 0/50 (Open; wrist YMAX ep49 255/255/255). Partial four-dimension Average SR **14.58%** (no complete Generalization cell; **not vs 14.88**). Official eps 700; in-progress 94/892 (10.54% micro). GPU3 refilled with `put_bottles_into_dustbin`. Native `--num_envs 5`. | not started |
| Xiaomi_Robotics_1 | `RoboDojo-sim-arx_x5-ee-0` | ee | **smoke gate passed**: `stack_bowls --eval-num 2` closed-loop 2/2 success on GPU6, `--num_envs 5`, `action_type=ee`, three-camera YMAX 255/254/253. Native 42-cell table still **0/42** (smoke stamp is 2 episodes; official cell needs 50). Elastic returned to G05-first (`sort_nesting_dolls_by_size_random` on GPU6). | not started |
