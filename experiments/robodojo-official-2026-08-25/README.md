...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **2/42**. Cells: `play_stacking_toy` 0/50, `classify_objects_by_language` 0/50. In-progress 18/294 (6.12%). `build_tower` already 3/5. `imitate_sorting_sequence` 49/50. GPU5 `fill_pen_holder`. Wrist YMAX ≫30. Native `num_envs=5`. | not started |
