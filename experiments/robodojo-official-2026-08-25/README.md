...
The seed-0 table is **2/42** as of 21:32 CST Aug 26 (`build_tower` 58%, `classify_objects_by_language` 0%). Do not compare that two-cell mean (29%) to the published 6.91%. Eight Isaac clients with untiled cameras; wrist videos still colorful. Early WS `TimeoutError`s during policy warmup recovered without stopping the sweep. Two completed Pi_05/G05 tables were
