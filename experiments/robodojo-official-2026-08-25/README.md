...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **28/42**. Last official cell: `sweep_blocks` 0/50. `hang_mugs_random` 0/25 done (std sibling not started, so not a new cell). Partial Average **12.78%** (**not vs 14.88**). GPU1 now `swap_T`. Wrist YMAX `hang_mugs_random` ep24 254/255/255. Native `--num_envs 5`. | not started |
