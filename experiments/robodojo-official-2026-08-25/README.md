...
| --- | --- | --- | --- | --- |
| Pi_05 | `RoboDojo-sim-arx_x5-joint-0` | joint | valid rerun (`num_envs=5`). Official cells **8/42**. New: `fill_pen_holder` 2/50 (wrists YMAX 251/246/247). Partial mean 10.75% not comparable to 6.91. GPU7 now `cover_blocks`. | not started |
...
The seed-0 table is **8/42** as of 23:38 CST Aug 26. Do not compare that eight-cell mean (10.75%) to the published 6.91%. Eight Isaac clients with untiled cameras; `fill_pen_holder` wrists colorful (YMAX 251/246/247). GPU1 `play_tic_tac_toe` can sit at 0% util between 5-env flushes because the horizon is 1400 steps. Early WS `TimeoutError`s during policy warmup recovered without stopping the sweep. Two completed Pi_05/G05 tables were
