...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **9/42**. New: `fill_pen_holder` 8/50. Partial four-dimension Average SR **6.00%** (Generalization has no completed cell; not vs 14.88). In-progress 76/595 (12.77% micro). `build_tower` 28/40. GPU5 idle after fill_pen, awaiting elastic refill. Native `--num_envs 5`. Wrist YMAX on `cover_blocks` ep49 and `fill_pen_holder` ep49 all ≫ 30. | not started |
| Xiaomi_Robotics_1 | `RoboDojo-sim-arx_x5-ee-0` | ee | queued; checkpoint resolves; SDPA fallback verified offline (`results/xiaomi-sdpa-preflight.json`); `stack_bowls --eval-num 2` now launches on the **next free GPU** before remaining G05 native cells | not started |
