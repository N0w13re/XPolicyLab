...
| --- | --- | --- | --- | --- |
| Pi_05 | `RoboDojo-sim-arx_x5-joint-0` | joint | valid rerun (`num_envs=5`). Official cells **30/42**. New: `stack_bowls` 19/50 (17/25 + random 2/25, YMAX 252/220/255). Partial mean 11.87%; in-progress 192/1684 at 11.40%. | not started |
...
The seed-0 table is **30/42** as of 04:56 CST Aug 27. Do not compare that thirty-cell mean (11.87%) to the published 6.91% until 42/42. Eight Isaac clients with untiled cameras. Measured ~169 ep/h over 10.0 h (~2.5 h left on Pi_05 at that rate). GPU5 finished `stack_bowls` and is now on `hang_mugs`. GPU1 `play_tic_tac_toe` can sit at 0% util between 5-env flushes because the horizon is 1400 steps. Early WS `TimeoutError`s during policy warmup recovered without stopping the sweep. Two completed Pi_05/G05 tables were
