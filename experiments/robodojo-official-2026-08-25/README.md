...
| --- | --- | --- | --- | --- |
| Pi_05 | `RoboDojo-sim-arx_x5-joint-0` | joint | valid rerun (`num_envs=5`). Official cells **24/42**. New: `arrange_largest_number` 1/50 (1/25+0/25, wrists YMAX 252/226/236). Partial mean 12.58%; in-progress 162/1445 at 11.21%. | not started |
...
The seed-0 table is **24/42** as of 04:05 CST Aug 27. Do not compare that twenty-four-cell mean (12.58%) to the published 6.91% until 42/42. Eight Isaac clients with untiled cameras; `arrange_largest_number` ep24 wrists colorful (YMAX 252/226/236). Measured ~158 ep/h over 9.1 h (~4.2 h left on Pi_05 at that rate). GPU1 `play_tic_tac_toe` can sit at 0% util between 5-env flushes because the horizon is 1400 steps. Early WS `TimeoutError`s during policy warmup recovered without stopping the sweep. Two completed Pi_05/G05 tables were
