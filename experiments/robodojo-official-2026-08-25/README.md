...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **12/42**. New: `build_tower` 35/50 (Precision). Partial four-dimension Average SR **14.58%** (no complete Generalization cell; **not vs 14.88**). In-progress 88/685 (12.85% micro). Native `--num_envs 5`. GPU0 `press_by_number`; GPU6 `sort_nesting_dolls_by_size_random`. | not started |
