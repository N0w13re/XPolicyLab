...
| --- | --- | --- | --- | --- |
| Pi_05 | `RoboDojo-sim-arx_x5-joint-0` | joint | valid rerun (`num_envs=5`). Official cells **9/42**. New: `organize_table` 2/50 (wrists YMAX 255/255/255). Partial mean 10.00% not comparable to 6.91. GPU4 now `pick_from_conveyor_by_image`. | not started |
...
The seed-0 table is **9/42** as of 00:08 CST Aug 27. Do not compare that nine-cell mean (10.00%) to the published 6.91%. Eight Isaac clients with untiled cameras; `organize_table` wrists colorful (YMAX 255/255/255). GPU1 `play_tic_tac_toe` can sit at 0% util between 5-env flushes because the horizon is 1400 steps. Early WS `TimeoutError`s during policy warmup recovered without stopping the sweep. Two completed Pi_05/G05 tables were
