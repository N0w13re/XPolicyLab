...
| G05 | `RoboDojo-sim-arx_x5-joint-0` | joint | **11/42**. New: `make_kong` 11/50 and `organize_table` 6/50. Partial four-dimension Average SR **6.33%** (no complete Generalization cell; not vs 14.88). In-progress 85/655 (12.98% micro). `build_tower` 32/45. Native `--num_envs 5`. Wrist YMAX on `organize_table` ep49 head/left/right 252/255/255. | not started |
