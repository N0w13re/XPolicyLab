...
additional_info="condition=P2-atomic,ckpt_name=${ckpt_name},action_type=${action_type}"
...
_has_remote_key="${RPENT_GPT_API_KEY:-${AZURE_OPENAI_API_KEY:-${OPENAI_API_KEY:-}}}"
_skip_local_qwen=0
...
if [[ "${EVAL_ENV_TYPE:-sim}" != "debug" ]] \
    ...
    source "${SCRIPT_DIR}/../Pi_05_Agent_P1_RPent/start_local_qwen.sh"
fi

echo "[MAIN] start P2 no-action policy server, port=${policy_server_port}"
