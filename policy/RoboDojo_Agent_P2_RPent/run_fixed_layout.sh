...
run_id=${ROBODOJO_RUN_ID:-p2-${task_name}-layout${layout//,/_}}
...
_remote_key="${RPENT_GPT_API_KEY:-${AZURE_OPENAI_API_KEY:-${OPENAI_API_KEY:-}}}"
if [[ "${_planner_backend}" != "azure" ]] \
    ...
    echo "[P2][ERROR] Selected planner backend has no API key." >&2
    exit 1
fi

bash "${XPL_ROOT}/scripts/run_robodojo_layout_range.sh" \
    RoboDojo_Agent_P2_RPent "${task_name}" "${layout}" \
