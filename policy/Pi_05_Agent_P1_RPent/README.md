...
- `ground`: locate one target and bind its bbox to a same-step world map
- `sample_world_xyz` / `query_world_map`: derive robust metric geometry
...
- `v1` is the current XPolicyLab RoboDojo/Pi_05 adaptation and remains the
  default. It loads the generic guide, exact task/seed curated resources when
  present, the legacy task recipe as an experimental prior, and the memory
  index. Missing curated resources remain supported and are recorded in trace.
...
`RPENT_APPROACH_CLEARANCE_M` controls the default hover offset above sampled
object geometry. `RPENT_PI05_EXECUTION_HORIZON` limits how many
