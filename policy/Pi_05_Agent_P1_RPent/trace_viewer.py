...
<div id="layout"><aside id="tools"></aside><main>
  ...
  <div class="controls">
    ...
    <button id="play-tool">Replay Tool</button>
    <button id="play-episode">Play Full Episode</button>
...
async function startPlayback(mode,index=selected){
  pausePlayback();
  const generation=++playbackGeneration;
  resumeMode=mode;
  if(mode==='tool'){
    setSelected(index);
    stopFrame=Math.max(bounds(manifest.tools[selected]).start,bounds(manifest.tools[selected]).end-1);
    seekFrame(bounds(manifest.tools[selected]).start);
  }else{
    setSelected(toolAtFrame(0));
  ...
  seekFrame(mode==='tool'?bounds(manifest.tools[selected]).start:0);
  syncFollowers(currentFrame(),true);
  playMode=mode;
...
document.querySelector('#play-episode').onclick=()=>startPlayback('episode');
document.querySelector('#pause').onclick=()=>{if(playMode==='paused'){startPlayback(resumeMode,selected)}else pausePlayback()};
document.querySelector('#prev').onclick=()=>{pausePlayback();seekFrame(currentFrame()-1)};
document.querySelector('#next').onclick=()=>{pausePlayback();seekFrame(currentFrame()+1)};
document.querySelector('#scrub').oninput=e=>{pausePlayback();seekFrame(+e.target.value)};
document.addEventListener('keydown',e=>{if(e.key==='ArrowLeft')document.querySelector('#prev').click();if(e.key==='ArrowRight')document.querySelector('#next').click();if(e.key===' '){e.preventDefault();document.querySelector('#pause').click()}});
