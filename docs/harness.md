...
> This is the reference documentation for **XPolicyLab**, the policy-serving
> harness ManipLoop is built on. It is kept verbatim because ManipLoop uses the
> harness unchanged: the same adapter contract, the same `policy/<POLICY>/`
> layout, the same data formats. The ManipLoop project itself — the P0–P3
> agentic-manipulation conditions — is documented in [../README.md](../README.md).
